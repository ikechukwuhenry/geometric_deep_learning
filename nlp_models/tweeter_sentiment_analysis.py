...
vocab = set(processed_positive_tweets + processed_negative_tweets)
...
len_positive = len(processed_positive_tweets)
len_negative = len(processed_negative_tweets)

priors = {
    "Positive": len(positive_tweets) / (len(positive_tweets) + len(negative_tweets)),
    "Negative": len(negative_tweets) / (len(positive_tweets) + len(negative_tweets))
}

print(f"Prior probabilities: {priors}")
log_priors = { cls: math.log(prior) for cls, prior in priors.items()}
# print(f"Log prior: {log_prior}")
print(f"Most common words in negative tweets: {negative_tweets_freq_dist.most_common(10)}")
pos_tweets_prob = {}
neg_tweets_prob = {}
lambda_of_word = {}
for word in vocab:
    pos_tweets_prob[word] = (positive_tweets_freq_dist[word] + 1) / (len_positive + vocab_size)
    neg_tweets_prob[word] = (negative_tweets_freq_dist[word] + 1) / (len_negative + vocab_size)
